document.observe("dom:loaded", function() {
	...
	if($('status')[$('status').selectedIndex].value == 1){
		...
		$('creareseo_discontinued_url').up(1).hide();
	} else {
		$('creareseo_discontinued').up(1).show();
		$('creareseo_discontinued_product').up(1).show();
		$('creareseo_discontinued_url').up(1).show();
	}
	...
	if(current != "301 Redirect to Custom Url"){
		$('creareseo_discontinued_url').up(1).hide();
	}
	...
	$('creareseo_discontinued').observe("change", function(e){
		...
		if($('creareseo_discontinued')[$('creareseo_discontinued').selectedIndex].text == "301 Redirect to Custom Url"){
			$('creareseo_discontinued_url').up(1).show();
		} else {
			$('creareseo_discontinued_url').up(1).hide();
		}
